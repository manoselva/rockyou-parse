...
if [ $# -lt 2 ]; then
    echo "Breach-Parse v2: A Breached Password Parsing Tool by Heath Adams"
    echo " "
    ...
    # grep for passwords matching the domain (e.g., @gmail.com)
    find "$breachDataLocation" -type f -not -path '*/\.*' -print0 | while read -d $'\0' file; do
        grep -a -E "$1" "$file" >>$passwords
        ((++file_Count))
        ProgressBar ${file_Count} ${total_Files}
    done

    # Remove any blank lines from the output (if any)
    sed -i '/^$/d' $passwords
fi
...
echo # newline
echo "Passwords extracted to $passwords"
